class StudioVersion < ApplicationRecord
  ...
  OPTIONS = { "radius" => %w[12px 18px 26px 32px], "font-body" => [ "DM Sans, sans-serif" ], "font-heading" => [ "Playfair Display, serif", "DM Sans, sans-serif" ], "motion" => %w[none subtle standard] }.freeze
  ...
  def css
    ...
    ":root{#{values}}" + (tokens["motion"] == "none" ? "*,*::before,*::after{animation:none!important;transition:none!important;scroll-behavior:auto!important}" : "")
  end
  def contrast_errors
  ...
  def safe_settings
    unless settings.is_a?(Hash) && (settings.keys - %w[tokens pages editorial_resets]).empty? && tokens.is_a?(Hash) && settings.fetch("pages", {}).is_a?(Hash)
