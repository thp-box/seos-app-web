...
RSpec.describe "Footer et accueil maquette", type: :request do
  it "affiche les quatre rubriques, les CGU et le centre légal" do
    ...
    expect(doc.at_css('.site-footer a[href="/legal/cgu"]').text).to eq("Règles et CGU")
    expect(doc.at_css('.site-footer a[href="/legal/mentions-legales"]')).to be_present
  ...
  it "affiche le document publié avec une navigation légale active" do
    ...
    expect(Nokogiri::HTML(response.body).at_css('.legal-sidebar a[aria-current="page"]').text).to eq("Règles et CGU")
  end
